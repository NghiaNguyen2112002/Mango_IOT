...
const byte request_nito[] =    {0x01, 0x03, 0x00, 0x1e, 0x00, 0x01, 0xe4, 0x0c};
const byte request_photpho[] =  {0x01, 0x03, 0x00, 0x1f, 0x00, 0x01, 0xb5, 0xcc};
const byte request_kali[] =     {0x01, 0x03, 0x00, 0x20, 0x00, 0x01, 0x85, 0xc0};

DHT dht(DHT_PIN, DHTTYPE);
SoftwareSerial rs485_serial(NPK_RX_PIN, NPK_TX_PIN);
...
unsigned char IN_ReadNito(void){
  ...
  if(rs485_serial.write(request_nito, sizeof(request_nito)) == 8){
    for(unsigned char i = 0; (i < 200) && !rs485_serial.available(); i++);
    for(unsigned char i = 0; i < 7; i++){
      Serial.print(value[i] = rs485_serial.read(), HEX); 
    }
    Serial.println("");
